...
class XenditWebhookHandler {
    ...
    public function verify() {
        ...
        if (!$webhookSecret || !$callbackToken || !hash_equals($webhookSecret, $callbackToken)) {
            error_log('Xendit webhook validation failed: ' . ($callbackToken ? 'Token mismatch' : 'Token not received'));
            return false;
        }
        return true;
    }
    ...
    public function getPaymentId($data) {
        if (isset($data->event) && $data->event === 'invoice.paid' && isset($data->data->status) && $data->data->status === 'PAID') {
